...
<?php 
		...
		echo '<pre>'; print_r($column); echo '</pre>'; */
			...
			echo "<br><div id='rec4'><button id='load-rec-button-4' class='button-success pure-button' onclick='loadRec4()' value='".$column_movietitle_lm[$randarray_m[1]]."'>Add ".$rand_rec_fname." ".$rand_rec_lname."'s suggestion to your list</button></div></div>";
			...
			$insert_movie1="INSERT INTO output (user_id, order_no, movie_title, rating, review, clicked, timed, rec_first_name, rec_last_name, rec_race, rec_gender) VALUES ('$user_id', '$index1', '$column_movietitle_esc1', '$column_rating_lm[$x]', '$column_review_esc1', 0, 0, '$column_first_esc1', '$column_last_esc1', '$column_race_esc1', '$column_gender_esc1')";
		...
		$rand_sql="select fname.first_name, lname.last_name, fname.race, fname.gender FROM fname INNER JOIN lname ON fname.race=lname.race WHERE ( CASE WHEN '$randkey_name' <= '605' THEN fname.race='White' WHEN '$randkey_name' > '605' && '$randkey_name' <= '740' THEN fname.race='Black' WHEN '$randkey_name' > '740' && '$randkey_name' <= '935' THEN fname.race='Asian' ELSE fname.race='Hispanic' END) ORDER BY RAND() LIMIT 1";
			...
			echo "<br><div id='rec5'><button id='load-rec-button-5' class='button-success pure-button' onclick='loadRec5()' value='".$column_movietitle_lm[$randarray_m[2]]."'>Add ".$rand_rec_fname." ".$rand_rec_lname."'s suggestion to your list</button></div></div>";

			//new
			$x=$randarray_m[2]; 
			$column_movietitle_esc2=mysqli_real_escape_string($conn, $column_movietitle_lm[$x]);
			$column_review_esc2=mysqli_real_escape_string($conn, $column_review_lm[$x]);
			...
			$insert_movie2="INSERT INTO output (user_id, order_no, movie_title, rating, review, clicked, timed, rec_first_name, rec_last_name, rec_race, rec_gender) VALUES ('$user_id', '$index2', '$column_movietitle_esc2', '$column_rating_lm[$x]', '$column_review_esc2', 0, 0, '$column_first_esc2', '$column_last_esc2', '$column_race_esc2', '$column_gender_esc2')";
